...
class QueryInterface:
    ...
    def main_menu_loop(self):
        self.__explicar_menu()
        query = self.__pedir_query()
        while query != "/q":
            results = self.queryProcessor.process(query)
            self.__show_query_results(results)
            self.__print_space_between_result_and_new_query()
            query = self.__pedir_query()
        self.__print_end()

    def __explicar_menu(self):
        print("--- Menu de querys --- \r \r")
        print("--- Ingrese en el input la query a realizar y aprete enter ---")
        print("--- Para salir ingrese: /q ---")

    def __pedir_query(self) -> str:
        return input("query: ")

    def __show_query_results(self, results):
        print(" \r Resultados del query: \r")
        for r in results:
            print(str(r))

    def __print_space_between_result_and_new_query(self):
        print("\r \r")
